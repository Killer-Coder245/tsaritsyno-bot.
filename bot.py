import os
import json
from datetime import datetime
from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup
from telegram.ext import Application, CommandHandler, CallbackQueryHandler, MessageHandler, filters, ConversationHandler
from flask import Flask, request
import asyncio
import threading

# ========== КОНФИГУРАЦИЯ ==========
TOKEN = '8539489977:AAFCgiAz7M9CH7yssMkdBo0BLH1310Tiuo0'  # Токен из переменных окружения


DATA_FILE = "races_data.json"

# ========== ЗАГРУЗКА ДАННЫХ ==========
def load_data():
    if os.path.exists(DATA_FILE):
        with open(DATA_FILE, 'r') as f:
            return json.load(f)
    # Начальные данные (сокращённо, добавьте все строки из предыдущего кода)
    return [
        [2022, 7, 30, 49], [2022, 8, 6, 37], [2022, 8, 13, 40],
        # ... добавьте все данные из таблицы выше
    ]

def save_data(data):
    with open(DATA_FILE, 'w') as f:
        json.dump(data, f, indent=2)

raw_data = load_data()

# ========== ТАБЛИЦА ==========
def build_table():
    rows = []
    current_year = None
    year_sum = 0
    prev_count = None
    
    for year, month, day, count in sorted(raw_data, key=lambda x: (x[0], x[1], x[2])):
        date_str = f"{day:02d}.{month:02d}.{year}"
        if year != current_year:
            current_year = year
            year_sum = 0
            prev_count = None
        
        if prev_count is None:
            diff = "-"
        else:
            diff = count - prev_count
        
        year_sum += count
        sum_x5 = year_sum * 5
        rows.append([date_str, count, diff, sum_x5, sum_x5])
        prev_count = count
    
    return rows

def format_table(rows, year_filter=None):
    lines = ["<pre>"]
    header = f"{'Дата':<12} {'Участ':<6} {'Приб':<6} {'Сумма×5':<8} {'Дубль×5':<8}"
    lines.append(header)
    lines.append("-" * len(header))
    
    for row in rows:
        date_str, count, diff, sum1, sum2 = row
        year = int(date_str.split('.')[-1])
        if year_filter and year != year_filter:
            continue
        diff_str = f"{diff:+d}" if isinstance(diff, int) else diff
        lines.append(f"{date_str:<12} {count:<6} {diff_str:<6} {sum1:<8} {sum2:<8}")
    
    lines.append("</pre>")
    text = "\n".join(lines)
    return text[:4000] if len(text) > 4000 else text

# ========== БОТ ==========
app_bot = None
flask_app = Flask(__name__)

async def start(update: Update, context):
    keyboard = [[InlineKeyboardButton("📊 Вся таблица", callback_data="show_all")],
                [InlineKeyboardButton("📅 За год", callback_data="show_year")],
                [InlineKeyboardButton("➕ Добавить забег", callback_data="add_run")]]
    await update.message.reply_text("🏃 Забеги в Царицыно\nДанные с 2022 года", reply_markup=InlineKeyboardMarkup(keyboard))

async def button_handler(update: Update, context):
    query = update.callback_query
    await query.answer()
    if query.data == "show_all":
        await query.edit_message_text(format_table(build_table()), parse_mode="HTML")
    elif query.data == "show_year":
        years = sorted(set(row[0].split('.')[-1] for row in build_table()))
        keyboard = [[InlineKeyboardButton(str(y), callback_data=f"year_{y}")] for y in years]
        await query.edit_message_text("Выберите год:", reply_markup=InlineKeyboardMarkup(keyboard))
    elif query.data.startswith("year_"):
        year = int(query.data.split("_")[1])
        await query.edit_message_text(format_table(build_table(), year_filter=year), parse_mode="HTML")
    elif query.data == "add_run":
        await query.edit_message_text("Введите /add для добавления записи")

# Добавление записи
YEAR, MONTH, DAY, COUNT_STATE = range(4)

async def add_start(update: Update, context):
    await update.message.reply_text("Введите **год** (например: 2026):")
    return YEAR

async def add_year(update: Update, context):
    context.user_data['year'] = int(update.message.text)
    await update.message.reply_text("Введите **месяц** (1-12):")
    return MONTH

async def add_month(update: Update, context):
    context.user_data['month'] = int(update.message.text)
    await update.message.reply_text("Введите **день** (суббота):")
    return DAY

async def add_day(update: Update, context):
    context.user_data['day'] = int(update.message.text)
    await update.message.reply_text("Введите **количество участников**:")
    return COUNT_STATE

async def add_count(update: Update, context):
    raw_data.append([context.user_data['year'], context.user_data['month'], context.user_data['day'], int(update.message.text)])
    raw_data.sort(key=lambda x: (x[0], x[1], x[2]))
    save_data(raw_data)
    await update.message.reply_text(f"✅ Добавлено!")
    return ConversationHandler.END

async def cancel(update: Update, context):
    await update.message.reply_text("Отменено")
    return ConversationHandler.END

# ========== ВЕБХУКИ ДЛЯ RENDER ==========
@flask_app.route(f'/{TOKEN}', methods=['POST'])
async def webhook():
    update = Update.de_json(request.get_json(force=True), app_bot.bot)
    await app_bot.process_update(update)
    return 'ok'

@flask_app.route('/')
def home():
    return "Бот работает!"

def setup_webhook():
    webhook_url = f"https://{os.environ.get('RENDER_EXTERNAL_HOSTNAME')}/{TOKEN}"
    app_bot.bot.set_webhook(webhook_url)

def main():
    global app_bot
    app_bot = Application.builder().token(TOKEN).build()
    app_bot.add_handler(CommandHandler("start", start))
    app_bot.add_handler(CallbackQueryHandler(button_handler))
    
    conv_handler = ConversationHandler(entry_points=[CommandHandler("add", add_start)], states={YEAR: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_year)], MONTH: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_month)], DAY: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_day)], COUNT_STATE: [MessageHandler(filters.TEXT & ~filters.COMMAND, add_count)]}, fallbacks=[CommandHandler("cancel", cancel)])
    app_bot.add_handler(conv_handler)
    
    threading.Thread(target=lambda: flask_app.run(host='0.0.0.0', port=int(os.environ.get('PORT', 8080)))).start()
    asyncio.run(setup_webhook())

if __name__ == "__main__":
    main()
